...
class NG_RC():
	...
	def __init__(
		...
		self._k = k                         # number of datapoints used for ngrc.
		self._s = s                         # spacing between the datapoints.
		self._orders = orders               # orders used to create unqiue monomials, e.g orders=[1,2,4], of linear state vectors.
		...
		self._mode = mode                   # Define on what the ngrc should be trained on. Options: 'coordinates,'differences' and 'inference'.
		self._order_type = order_type       # Specific order_type. Options: 'all' for all unqiue monomials of certain orders. 'interactions' only interacting terms of certain orders.
	...
	def create_train_X_y(
		...
		"""Creates and/or assigns the training and target data given the choosen ngrc 'mode'= {'coordinates','differences','inference'}.
		...
		Args:
			...
			mode: 'inference'   -> Allows to define a custom taget data on which the ngrc is trained on.
		...
		if mode == 'coordinates':
			...
			if target_data is None:
				raise ValueError(
					('Target data for inference mode is not specified')
					)
			else:
			...
			raise ValueError(
				('Mode configuration Error: choose betwenn coordinates , differences and inference ')
				)
	...
	def linear_expansion(self,
		...
		if functional is False:
			...
			linear_states = pre_expanded_states.reshape(pre_expanded_states.shape[0],pre_expanded_states.shape[1]*pre_expanded_states.shape[2])
	...
	def nonlinear_expansion(self,
		...
		if functional is False:
			...
			if self._order_type == 'all':
				...
				raise ValueError('specifiy order_type correctly. \'all\' standart for using all combinations. \'interactions\' for only interactions terms in dictonary.')
			

			data_lenght = len(self._states)
			nonlinear_states = np.empty((data_lenght, len(self._dictionary)))
	...
	def expanding_states(self,
		...
		if functional is False:
			...
			if self._expanding_orders is None:
				...
				data_lenght = self._states.shape[0]
				state_dimension = self._states.shape[1]
				expanded_states = np.empty((data_lenght,state_dimension*len(self._expanding_orders)))

				for i in range(len(self._expanding_orders)):
					expanded_states[:, i*state_dimension:(i+1)*state_dimension] = self._states**self._expanding_orders[i]
		...
		if functional:
			...
			if self._expanding_orders is None:
				...
				data_lenght = input_data.shape[0]
				...
				if self._expanding_orders is not None:
					...
					for i in range(len(self._expanding_orders)):
						expanded_states[:, i*state_dimension:(i+1)*state_dimension] = input_data**self._expanding_orders[i]
	...
	def fit(self,
           X: np.array,
           y_target: Optional[np.array]=None
           )-> None:
		
		# train. defined in linear_expension
		# reg_param, regression parameter.
		# set w_out_fit_flag= [1,2,n] for specifc higher orders of the reservoir vectors, see r_to_generalized_r.
		# regression_type: some sklearn.linear_model functions 
		# --> max_iter=1000,fit_intercept=False,normalize=False,tol=0.001, varibales belong to sklearn.linear_model functions 
		...
		self._w_out = np.linalg.solve(
					X_states.T @ X_states + self._regression_parameter * np.eye(X_states.shape[1]),
					X_states.T @ y_target).T

	def predict(self,
				steps: int,
				starting_series: Optional[np.array]=None
				) -> np.array:
		...
		Args:
			...
			steps: int                  -> Specifiy the number of prediction steps to be made.
		...
		Returns:
			...
			If error occurs during prediction, e.g. if trajectory divereges, np.nan is returned.

		"""
		
		if starting_series is not None and starting_series.shape[0] < (self._k-1)*self._s+1:
				raise ValueError(
					('Starting series not correctly specified, make sure that it has a least (k-1)*s+1={} data points.'.format((self._k-1)*self._s+1))
					)
	...
	def inference(self,
		...
		if len(data) < (self._k-1)*self._s:
			raise('Data lenght does not match minimal warmup time required, needs to be a least (k-1)*s+1={} data points.'.format((self._k-1)*self._s+1))
